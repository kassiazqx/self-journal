...
│   └── reset.ts            reset + auth-state 脚本入口，供 globalSetup 调用
└── global.setup.ts         若本地已配置 `.env.e2e.local`，自动准备 reset fixture + auth state
...
│   └── create-auth-state.mjs  生成两套 Playwright 登录态文件
...
### 4.60 浏览器实时语音已移除，不再作为未来接缝（2026-04-28 更新）

**背景：** 用户手测发现 Web Speech API 路径存在“先手打再点语音时，原文字可能被顶掉”的问题；同时产品方向已明确偏向“上传录音 / 上传视频 / 再转文字”，不再继续投资浏览器实时语音。

**已处理：**
- `HomePage` 已移除浏览器语音按钮
- `useSpeechRecognition.js` 已删除
- 语音录制脉冲样式已删除

**当前原则：**
- 未来若做讯飞 / 上传录音 / 上传视频 / 再转文字，需新建媒体上传与转写链路
- 不要再恢复 Web Speech API 旧入口，也不要把它当成未来架构接缝
...
- 2026-04-29 · 协调session · Persistence Trust Batch A 文档收口：确认本批实际完成到 `Task5`；原 `Task6`“RecordDetail 最小分段渲染”不再按旧 plan 执行，已拆出并升级为后续“文本段落规则统一 + 标注/富文本一体化”专项；plan / sync-card 已同步改口
