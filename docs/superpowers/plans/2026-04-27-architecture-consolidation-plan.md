...
> 状态：P0 已落地，P1 主收口已完成且除一项桌面低优先级残余外已完成手工验证，P3 代码 / 数据 / 手工验证已完成；`legacy_only_count` 审计、历史回填、`full_conversation` 删列均已完成，当前主剩余已转向 P4 UI / APK 与后续细调。
...
| 项 | 说明 | 状态 |
|---|---|---|
| HomePage 桌面首次拖选菜单 | `MouseUpPlugin` 已收口到 `sync -> timeout-0 -> rAF`；仍留一项低优先级残余，不阻塞主线 | 低优先级残余 |
| 标注交互稳定性 | 高亮 / 下划线 / 加粗、移动端拖拽、只读页回归均已完成手工矩阵验证 | 已手工验证通过 |
| 编辑页交互可靠性 | EditEntryPage 桌面 / 移动端交互与保存链已验证 | 已手工验证通过 |
...
| 项 | 说明 | 状态 |
|---|---|---|
| 提取触发口径统一 | 只保留手动提取 + 回顾信前批量提取两种模式 | 已手工验证通过 |
| fullText 数据源统一 | 所有提取/回顾信/记忆相关读取统一走 `conversations` 主路径的完整 fullText | 已手工验证通过 |
| 记忆文本构建收口 | `Settings` 改用按 `nodeType` 精确映射的记忆文本 builder | 已手工验证通过 |
| 回顾信富内容收口 | `shared fullText + 结构化补充字段`，不丢用户后修订内容 | 已手工验证通过 |
| `AIConversation` / `full_conversation` 旧链清理 | 先移除运行时代码依赖；DB 列仅在 `legacy_only_count` 审计 + 回填/放弃确认后清理 | 已完成（审计 9 → 回填 → 0 → 删列） |
| AI 提取字段质量 | 在输入统一后，已完成摘要 / 情绪 / 需求等输出手测评估 | 已手工验证通过 |
| 跨对话记忆质量 | Settings 改读 `conversations` 后，已完成 `rolling_summary` 手测评估 | 已手工验证通过 |
