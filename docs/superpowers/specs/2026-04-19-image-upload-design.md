...
RecordsPage 挂载时：
  ...
  if (failed.length > 0) {
    ...
    // 用户点击 banner → 跳转到对应记录
    // 清除时机：后台重新上传成功后，在上传成功的同一回调中（§4 步骤 d），
    //           从 image_upload_failed 列表中移除该 entryId；不依赖用户手动操作
  }
```
...
6. 保存后：跳转立即发生，图片后台上传；下次进入记录列表时出现缩略图 + 线条图标 + 数量（后台上传不触发当前页面 state 刷新）
7. 图片上传失败时：**当场弹全局 Toast 通知**，不需要用户进详情页才发现；App 关闭期间失败则下次打开记录列表时显示 Banner
8. 点进详情页，原始记录流区块：文字下方出现图片宫格
9. 详情页觉察对话区（ai_prompt / ai_answer 流式文字）位置不变，样式不变
