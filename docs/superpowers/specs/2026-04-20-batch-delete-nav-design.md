...
- 未全选（含部分选中）：空心圆圈
- 已全选：金色填充对勾
- 点击 = 全选 / 取消全选 当天**当前视图中**所有条目（FilterBar 过滤中时，仅作用于可见条目）
...
| FilterBar | 进入多选模式时**不收起** FilterBar；「全选当天」作用于当前视图可见条目，FilterBar 过滤中时只选可见部分（行为符合预期，无需重置） |
...
**批量多选：**
1. 长按任意日记条目 600ms → 进入多选模式，该条目自动选中，顶栏变为「取消 / 已选1条 / 删除（金色）」
2. 点击条目 → checkbox 切换选中，顶栏计数更新
3. 点日期组「全选当天」→ 当天可见条目全选；再点 → 全取消
4. FilterBar 过滤中时，「全选当天」仅作用于当前可见条目
5. 选中 N 条后点「删除」→ 底部弹出确认 sheet → 确认后删除并刷新列表
6. 点「取消」或删除成功后退出多选模式
7. 回顾信固定卡片不出现 checkbox，不可选中
